...
AS SELECT S1.NetID AS student1NetID,S2.NetID AS student2NetID
FROM TEMP3 AS S1,TEMP3 AS S2
WHERE S1.NetID <> S2.NetID AND S1.Breed == S2.Breed;
...
FROM BITES,BITES AS BITES2
WHERE BITES.Biter == BITES2.Biter AND BITES.Bitee == '2' AND BITES2.Bitee == '3';

CREATE TABLE ANSWER4
AS SELECT BITES.SerialNumber,DOG.NNumber
FROM DOG,TEMP4 AS BITES
WHERE BITES.SerialNumber == DOG.SerialNumber;
...
FROM BITES,BITES AS BITES2
WHERE BITES.Biter == '5' AND BITES2 == '6';

CREATE TABLE ANSWER7
AS SELECT DISTINCT DOG.SerialNumber AS SerialNumber,DOG.NNumber AS NNumber
FROM DOG,TEMP7,TEMP8
WHERE TEMP7.SerialNumber == DOG.SerialNumber OR TEMP8.SerialNumber == DOG.SerialNumber;
